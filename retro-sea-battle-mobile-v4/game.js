...
const modeButtonsButton = document.getElementById("mode-buttons");
const modeTouchButton = document.getElementById("mode-touch");
...
function syncStartButton() {
  ...
  startButton.textContent = game.status === "running" ? "Идет раунд" : "Старт / рестарт";
  startButton.disabled = game.status === "running";
}

function syncControlModeUi() {
  if (modeButtonsButton) {
    modeButtonsButton.classList.toggle("mode-button--active", game.controlMode === "buttons");
  }
  if (modeTouchButton) {
    modeTouchButton.classList.toggle("mode-button--active", game.controlMode === "touch");
  }
  if (touchControls) {
    touchControls.classList.toggle("touch-controls--hidden", game.controlMode !== "buttons");
  }
  if (canvas) {
    canvas.classList.toggle("canvas--touch-aim", game.controlMode === "touch");
  }
}
...
function updateCrosshairFromPointer(clientY) {
  const rect = canvas.getBoundingClientRect();
  const relativeY = (clientY - rect.top) / rect.height;
  const canvasY = relativeY * game.height;
  game.crosshair.y = Math.max(70, Math.min(game.height - 80, canvasY));
}
...
function drawMessage() {
  ...
  if (game.status === "ready") {
    ctx.fillText("Нажми старт или Enter, чтобы начать", game.width / 2, 268);
    ctx.font = "18px Trebuchet MS";
    ctx.fillText("На телефоне выбери режим Кнопки или Палец", game.width / 2, 306);
    ctx.fillText("В режиме Палец прицел двигается касанием по полю", game.width / 2, 338);
  } else {
    ctx.fillText(`Счет: ${String(game.score).padStart(4, "0")}`, game.width / 2, 270);
    ctx.font = "18px Trebuchet MS";
    ctx.fillText("Нажми старт или Enter для нового раунда", game.width / 2, 308);
  }
...
function bindHoldControl(button, keyCode) {
  const activate = (event) => {
    event.preventDefault();
    ensureAudio();
    game.keys.add(keyCode);
  };

  const release = (event) => {
    event.preventDefault();
    game.keys.delete(keyCode);
  };
...
canvas.addEventListener("pointerdown", (event) => {
  ...
  updateCrosshairFromPointer(event.clientY);
});
...
canvas.addEventListener("pointermove", (event) => {
  ...
  if (game.touchPointerId !== event.pointerId) {
    return;
  }
  event.preventDefault();
  updateCrosshairFromPointer(event.clientY);
...
if (modeButtonsButton) {
  modeButtonsButton.addEventListener("click", () => setControlMode("buttons"));
}

if (modeTouchButton) {
  modeTouchButton.addEventListener("click", () => setControlMode("touch"));
}

controlButtons.forEach((button) => {
  const control = button.dataset.control;
  if (control === "up") {
    bindHoldControl(button, "ArrowUp");
  } else if (control === "down") {
    bindHoldControl(button, "ArrowDown");
