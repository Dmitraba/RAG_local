...
function playSound(kind) {
  ...
  if (kind === "start") {
    tone({ frequency: 210, duration: 0.12, type: "triangle", volume: 0.22, slide: 280 });
    setTimeout(() => tone({ frequency: 280, duration: 0.14, type: "triangle", volume: 0.2, slide: 360 }), 80);
    return;
  }

  if (kind === "fire") {
    tone({ frequency: 116, duration: 0.32, type: "sawtooth", volume: 0.26, slide: 72 });
    setTimeout(() => tone({ frequency: 180, duration: 0.1, type: "triangle", volume: 0.12, slide: 120 }), 70);
    return;
  }

  if (kind === "hit") {
    noiseBurst(0.18, 0.24);
    tone({ frequency: 220, duration: 0.14, type: "square", volume: 0.26, slide: 420 });
    setTimeout(() => tone({ frequency: 360, duration: 0.18, type: "triangle", volume: 0.22, slide: 660 }), 40);
    setTimeout(() => tone({ frequency: 120, duration: 0.42, type: "sawtooth", volume: 0.14, slide: 80 }), 80);
    return;
  }

  if (kind === "miss") {
    tone({ frequency: 170, duration: 0.2, type: "triangle", volume: 0.19, slide: 110 });
    setTimeout(() => noiseBurst(0.12, 0.1), 40);
    return;
  }

  if (kind === "win") {
    tone({ frequency: 320, duration: 0.16, type: "triangle", volume: 0.22, slide: 460 });
    setTimeout(() => tone({ frequency: 460, duration: 0.22, type: "triangle", volume: 0.2, slide: 700 }), 120);
    return;
  }

  if (kind === "lose") {
    tone({ frequency: 220, duration: 0.18, type: "sawtooth", volume: 0.2, slide: 160 });
    setTimeout(() => tone({ frequency: 160, duration: 0.26, type: "sawtooth", volume: 0.18, slide: 110 }), 100);
  }
}
